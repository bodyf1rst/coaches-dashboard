...
export class IntroVideosComponent implements OnInit {
  ...
  getFreshData() {
    const videoExits = this.dataService.introVideos?.['generalVideos']?.videos?.length > 0;
    if (videoExits) {
      this.clearSearch();
      return;
    }
  ...
  loadMoreVideos(tabView: any) {
    const currentPage = this.dataService.introVideos?.generalVideos?.page;
    const totalPage = this.dataService.introVideos?.generalVideos?.total_page;
