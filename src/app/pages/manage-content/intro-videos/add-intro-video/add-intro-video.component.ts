...
@Component({
  selector: 'app-add-intro-video',
  templateUrl: './add-intro-video.component.html',
  styleUrls: ['./add-intro-video.component.scss']
})
export class AddIntroVideoComponent implements OnInit {
      ...
      this.dataService.httpService.postApiFileData(apiName, appendedFormData).then((res: any) => {
        ...
        if (this.dataService.introVideos[listName]?.videos?.length) {
          ...
          if (videoIndex > -1) {
            ...
            this.dataService.router.navigateByUrl('/manage-intro-videos')
          }
        } else {
          this.dataService.router.navigateByUrl('./intro-video-detail/' + this.videoData.id)
        }
