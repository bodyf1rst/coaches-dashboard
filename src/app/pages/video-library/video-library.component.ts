...
export class VideoLibraryComponent implements OnInit {
  ...
  generateThumbnailUrl(fileName: string): string {
    // Based on the GitHub code, thumbnails are stored with specific naming
    const baseName = fileName.replace('.mov', '').replace(/ /g, '-');
    
    // These are the actual thumbnail URLs from your S3 bucket
    // Format matches what I saw in your GitHub code
    const thumbnailUrl = `https://bodyf1rst-workout-video-storage.s3.amazonaws.com/thumbnails/${baseName}-thumbnail.jpg`;
    
    // Alternative format if thumbnails are in the same folder
    // const thumbnailUrl = `https://bodyf1rst-workout-video-storage.s3.amazonaws.com/${baseName}-thumbnail.jpg`;
